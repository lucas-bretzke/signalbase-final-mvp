    ...
    <div className="product-shell">
      <aside className="sidebar">
        <button className="brand" onClick={() => navigate('dashboard')} aria-label="Ir para o Dashboard">
          ...
          <span><strong>signalbase</strong><small>lead intelligence</small></span>
        </button>
      ...
      <div className="workspace">
        <header className="topbar">
          <div className="mobile-brand"><span className="brand-mark"><i /><i /><i /></span><strong>signalbase</strong></div>
          <div className="crumbs"><span>SignalBase</span><b>/</b><strong>{pageTitle(route.view)}</strong></div>
...
function NewSearchPage({ onCreated }: { onCreated: (search: LeadSearch) => void }) {
  ...
  return (
    <>
      <section className="page-heading"><span className="eyebrow">Nova Busca</span><h1>Encontre os leads certos para o seu mercado.</h1><p>Defina o recorte. A SignalBase encontra as empresas e enriquece até atingir sua meta de contatos válidos.</p></section>
...
function routePath(route: RouteState): string { if (route.view === 'new-search') return '/searches/new'; if (route.view === 'searches') return '/searches'; if (route.view === 'review' && route.searchId) return `/searches/${route.searchId}`; if (route.view === 'detail' && route.searchId && route.resultId) return `/searches/${route.searchId}/results/${route.resultId}`; if (route.view === 'exports') return `/exports${route.searchId ? `?search=${encodeURIComponent(route.searchId)}` : ''}`; return '/'; }
